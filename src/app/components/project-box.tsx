...
const ProjectBox: React.FC<ProjectBoxData> = ({
  img, status, title, content, url, github, skill, preview,
}) => {
  ...
  const handleShare = async (url: string) => {
    const shareData = { title: "Check out this Project", url: url }
    try {
      if (navigator.share) {
    ...
    <div onClick={() => setOpen((prev) => !prev)}
      ...
      <div className="flex md:flex-row flex-col gap-3 p-2">
        <div className="basis-[22%] p-1 select-none">
          <Image className="rounded-md md:h-[130px] h-[200px] w-full object-cover"
            src={img} alt="Project Image" width={200} height={200} />
        </div>
            ...
            <div className="flex gap-2 items-center truncate">
              ...
              {status ? (
                <div className="select-none font-medium text-xs w-fit px-1.5 py-0.5 gap-0.5 rounded-md flex items-center bg-green-400/10 text-green-400">
                  <span className="animate-pulse"><GoDotFill /></span> Running
                </div>
              ) : (
                <div className="select-none font-medium text-xs w-fit px-1.5 py-0.5 gap-0.5 rounded-md flex items-center bg-red-400/10 text-red-400">
                  <span className="animate-pulse"><GoDotFill /></span> Building
                </div>
              )}
            </div>
            <div className="select-none flex gap-2 px-2 text-base">
                ...
                <>
                  {show ? (
                    <InfoTipProjects text="Close">
                      <a onClick={(e) => { e.stopPropagation(); setShow((prev) => !prev) }}
                         target="_blank" className="cursor-pointer hover:text-zinc-400 transition-colors duration-100">
                        <FaEyeSlash />
                      </a>
                    </InfoTipProjects>
                  ) : (
              ...
              {url && (
                <InfoTipProjects text="Live">
                  <a target="_blank" className="hover:text-zinc-400 transition-colors duration-100"
              ...
              {github && (
                <InfoTipProjects text="Github">
                  <a onClick={(e) => { e.stopPropagation() }} target="_blank"
                     className="hover:text-zinc-400 transition-colors duration-100" href={github}>
                    <FiGithub />
                  </a>
                </InfoTipProjects>
              )}
      ...
      {/* Expandable skills/share section */}
      <AnimatePresence mode="wait">
        {open && (
          <motion.div
            initial={{ opacity: 0, height: 0 }}
            ...
            <div className="flex justify-between items-center md:py-2 py-3 px-3 transition-all duration-100">
              <div className="flex flex-wrap gap-1.5 select-none">
                {skill.map((skill, index) => (
                  <p key={index}
                     className="border border-zinc-700 px-2 py-0.5 rounded-md text-sm">{skill}</p>
                ))}
              </div>
              <div className="flex gap-4 items-center md:px-2 px-2.5 md:text-lg text-xl">
                <div className="cursor-pointer select-none hover:text-zinc-400 transition-colors duration-100"
                  onClick={(e) => { e.stopPropagation(); handleShare(url ? url : github) }}>
                  <LuShare />
                </div>
              </div>
            </div>
