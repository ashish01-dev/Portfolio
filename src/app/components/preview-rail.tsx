...
export function PreviewRail({
  ...
  const rowTemplate = items.length ? `repeat(${items.length}, 1.25rem)` : undefined;
    ...
    >
          ...
          return (
            ...
            >
              <motion.span
                ...
                transition={reduce ? { duration: 0 } : { type: "spring", stiffness: 360, damping: 32, mass: 0.6 }}
                className={cn(
                  "block bg-current",
      ...
      >
        {items.map((item) => (
          <div
            key={item.id}
            className={cn("relative flex h-5 items-center", isHorizontal ? "w-5 justify-center" : undefined)}
          >
            {item.id === displayedId ? (
              <div
                className={cn(
                  isHorizontal ? "absolute bottom-12 left-1/2 w-72 -translate-x-1/2" : "w-full max-w-sm",
                  previewClassName,
                )}
              >
                <motion.div
                  ...
                  <AnimatePresence mode="wait" initial={false}>
                    <motion.div
                      ...
                      exit={
                        reduce
                          ? { opacity: 0 }
                          : { opacity: 0, y: -2, filter: "blur(4px)", transition: { duration: 0.12, ease: [0.16, 1, 0.3, 1] } }
                      }
